...
def test_label_gaussian_isotropic_scale_factors(input_images):  # noqa: F811
...
def test_anisotropic_scale_factors(input_images):  # noqa: F811
    ...
    multiscale = to_multiscale(image, scale_factors, method=Methods.ITK_BIN_SHRINK)
    baseline_name = "x1y2z1_x2y2z2_x3y2z4/ITK_BIN_SHRINK"
    verify_against_baseline(dataset_name, baseline_name, multiscale)
...
def test_from_itk(input_images):  # noqa: F811
    import itk
    import numpy as np
    ...
    try:
        itk_image_to_multiscale(
            image, scale_factors=scale_factors, anatomical_axes=True
        )
        raise Exception(
            "Failed to catch expected exception for 2D image requesting anatomical axes"
        )
    except ValueError:
    ...
    image = itk.image_from_xarray(input_images[dataset_name])
    image.SetObjectName(
        str(input_images[dataset_name].name)
    )  # implicit in image_from_xarray in itk>v5.3rc04
